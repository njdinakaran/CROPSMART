...
function fetchCityState(city) {
    const cityData = cities.find(item => item.city.toLowerCase() === city.toLowerCase());
    if (cityData) {
        // document.getElementById('state').innerText = "State: " + cityData.state+"\n";
        findRegion(cityData.state);
    } else {
...
//display work for crop_recommendation
...
function displayCrops(region) {
    crops = cropMapping[region] || [];
    const cropsNames = crops; // Extracting crop names
    // console.log(cropsNames); // Logging crop names array
    // detailedInfo = cropsNames.map(cropName => {
    //     const crop = fullcropdata.find(c => c.name.toLowerCase() === cropName.toLowerCase());
    //     return crop ? `${crop.name}: ${crop.description}: ${crop.image}` : `${cropName}: Description not available`;
    // });
    detailedInfo = crops.map(cropName => fullcropdata.find(crop => crop.name === cropName))
                                .filter(crop => crop !== undefined);
            if (detailedInfo.length > 0) {
                showCrop(0);  // Show the first crop in the list
            }

    console.log("mydetail",detailedInfo);


    // const cropsElement = document.getElementById('crops');
    // cropsElement.innerText = crops.join(", ");
}
...
document.getElementById('pbutton').addEventListener('click', () => {
    currentIndex = (currentIndex === 0) ? crops.length - 1 : currentIndex - 1;
    showCrop(currentIndex);
});

document.getElementById('nbutton').addEventListener('click', () => {
    currentIndex = (currentIndex === crops.length - 1) ? 0 : currentIndex + 1;
    showCrop(currentIndex);
});
